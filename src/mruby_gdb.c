...
mrb_gdb_get_current(struct mrb_state *mrb)
{
	...
	sprintf(ret, "result={name=\"%s\",line=\"%d\",ciidx=\"%d\"}", filename, line, ciidx);
	return ret;
}
...
mrb_gdb_get_locals(struct mrb_state* mrb){
	...
	strcpy(ret, "locals=[");mrb->code_fetch_hook = NULL;
	for (i=0; i+1<local_len; i++) {
		...
		sprintf(buf, "{name=\"%s\",value=\"%s\",type=\"%s\"}", symname, v2_cstr, v2_classname);
		if(flg == 1){
			strcat(ret, ",");
		}
		strcat(ret,buf);
